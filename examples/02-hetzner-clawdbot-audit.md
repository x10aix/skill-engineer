# Example 02: Infrastruktur-Agent (Hetzner OpenClaw)
...
## Die Ausgangslage
Der Nutzer legte den Skill `hetzner-openclaw-access` vor. Dieser sollte Anweisungen für den asynchronen SSH-Zugriff und eine Task-Queue-Interaktion mit einem Server-Bot ("Clawdi") steuern.
...
2. **Vage Triggering-Logik (Kriterium 6):** Der Nutzer wollte, dass der Ziel-Agent automatisch auf den Chat-Befehl `/sync-clawdi` reagiert. Diese Anweisung war jedoch tief im Text vergraben.
...
- **Harte Trigger:** Es wurden zwei separate, blind ausführbare Teil-Workflows definiert:
  - *Workflow A:* "Neue Aufgabe an Clawdi senden" (Triggert situativ bei Bedarf).
  - *Workflow B:* "Sync Clawdi" (Triggert exakt bei Eingabe von `/sync-clawdi` -> Lade Queue, arbeite Jobs ab, formatiere Markdown, push Queue).
