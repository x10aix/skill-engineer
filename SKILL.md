...
8a. **Static References:** Frameworks, checklists, templates that rarely change. → Design: Compact, no status tracking needed.
8b. **Dynamic References (Living Documents):** Inventories, lists, logs that grow during operation. → Design: MUST contain: Last-Updated date, maintenance rules, status column (✅/⏳/🔲). See [Reference Templates](references/reference-templates.md) for scaffolding patterns.
...
3. **Cross-Skill Delegation Pattern:** If Interview Metric 9 identified delegation partners, inject the following pattern into the generated skill's `<operational_rules>`:
   ```
   - NEVER [perform task X] yourself — delegate to `[skill-name]` with the following briefing:
...
- **Diagram Quality:** Are decision trees with ≥3 branches rendered as Mermaid diagrams (not ASCII art) for dual readability (user rendering + LLM parsing)?
